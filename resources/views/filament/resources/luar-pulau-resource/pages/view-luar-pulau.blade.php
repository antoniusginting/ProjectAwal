<x-filament-panels::page>
    <div class="p-6 bg-white dark:bg-gray-900 rounded-md shadow-md space-y-6 text-gray-900 dark:text-gray-200">
        @php
            ...
            if ($luarPulau->penjualanLuar) {
                $penjualanFiltered = $luarPulau->penjualanLuar;
            }

            // Hitung total berat dari pembelian dan penjualan yang sudah difilter
            ...
            $sisaPembelian = $totalBeratPembelianFiltered - $totalBeratPenjualanFiltered;
            $persenanPembelian =
                $totalStokDanBerat != 0 ? ($totalBeratPembelianFiltered / $totalStokDanBerat) * 100 : 0;
            $persenanPenjualan =
                $totalBeratPembelianFiltered != 0
                    ? ($totalBeratPenjualanFiltered / $totalBeratPembelianFiltered) * 100
                    : 0;

        @endphp

        {{-- Summary Dashboard - Layout 2 Baris --}}
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg mb-6 shadow-md border">
            ...
            <div>
                ...
                <div class="flex flex-row gap-4">
                    ...
                    <!-- Sisa Pembelian -->
                    <div
                        class="flex-1 text-center p-3 {{ $sisaPembelian >= 0 ? 'bg-teal-50 dark:bg-teal-900/20' : 'bg-red-50 dark:bg-red-900/20' }} rounded-lg">
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Sisa Pembelian</p>
                        <p
                            class="text-xl font-bold {{ $sisaPembelian >= 0 ? 'text-teal-600 dark:text-teal-400' : 'text-red-600 dark:text-red-400' }}">
                            {{ number_format($sisaPembelian, 0, ',', '.') }}
                        </p>
                    </div>
        ...
        <div class="mb-6" id="laporan-penjualan">
            ...
            <table class="w-full border border-collapse border-gray-300 dark:border-gray-700">
                ...
                <tfoot>
                    <tr class="bg-gray-100 dark:bg-gray-800 font-semibold">
                        <td colspan="5" class="border p-2 text-center border-gray-300 dark:border-gray-700 text-sm">
                            Total Berat:
                        </td>
                        <td class="border p-2 text-right border-gray-300 dark:border-gray-700 text-sm">
                            {{ number_format($totalBeratPenjualanFiltered, 0, ',', '.') }}
