<x-filament-panels::page>
    <div class="p-6 bg-white dark:bg-gray-900 rounded-md shadow-md space-y-6 text-gray-900 dark:text-gray-200">
        ...
        @else
            ...
            {{-- VIEW UNTUK TABEL LUAR --}}
            @php
                // Ambil data langsung dari relasi luars
                $luarsFiltered = collect();

                // Ambil semua data luars
                if ($silo->luars) {
                    $luarsFiltered = $silo->luars;
                }

                // Hitung total berat dari luars (netto)
                $totalBeratLuars = $luarsFiltered->sum('netto');

                // Data untuk pagination
                $laporanLuarsTotal = $luarsFiltered->count();

                // Hitung total berat dari stock luar
                $totalBeratStockLuar = $silo->stockLuar()->sum('quantity');

                // Hitung summary
                $totalStokDanBeratLuar = $silo->stok + $totalBeratStockLuar;
                $stokSisaLuar = $totalStokDanBeratLuar - $totalBeratLuars;
                $persenanLuar = $totalStokDanBeratLuar != 0 ? ($totalBeratLuars / $totalStokDanBeratLuar) * 100 : 0;
            @endphp
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-lg mb-6 shadow-md border">
                ...
                <div class="flex flex-row gap-4">
                    ...
                    <div class="flex-1 text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Masuk</p>
                        <p class="text-xl font-bold text-green-600 dark:text-green-400">
                            {{ number_format($totalBeratStockLuar, 0, ',', '.') }}
                        </p>
                    </div>
                    ...
                    <div class="flex-1 text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Total Stok</p>
                        <p class="text-xl font-bold text-green-600 dark:text-green-400">
                            {{ number_format($totalStokDanBeratLuar, 0, ',', '.') }}
                        </p>
                    </div>
                    ...
                    <!-- Stok Sisa -->
                    <div
                        class="flex-1 text-center p-3 {{ $stokSisaLuar >= 0 ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-red-50 dark:bg-red-900/20' }} rounded-lg">
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Stok Sisa</p>
                        <p
                            class="text-xl font-bold {{ $stokSisaLuar >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
                            {{ number_format($stokSisaLuar, 0, ',', '.') }}
                        </p>
                    </div>
                    ...
                    <div
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Persenan</p>
                        <p
                            class="text-xl font-bold {{ $persenanLuar >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
            ...
            {{-- Tabel Data Luars --}}
            <div class="mb-6" id="laporan-luars">
                <div class="flex justify-between items-center mb-3">
                    <div class="flex items-center gap-3">
                        <h3 class="text-lg font-semibold">Laporan Data Luar</h3>
                    </div>
                </div>
                ...
                <table class="w-full border border-collapse border-gray-300 dark:border-gray-700">
                    <thead>
                        <tr class="bg-gray-100 dark:bg-gray-800">
                            <th class="border p-2 border-gray-300 dark:border-gray-700 text-sm">Tanggal</th>
                            <th class="border p-2 border-gray-300 dark:border-gray-700 text-sm">Nama</th>
                            <th class="border p-2 border-gray-300 dark:border-gray-700 text-sm">Kode Segel</th>
                            <th class="border p-2 border-gray-300 dark:border-gray-700 text-sm">Netto</th>
                        </tr>
                    </thead>
                    <tbody id="luars-tbody">
                        @php $luarsIndex = 0; @endphp
                        @forelse($luarsFiltered as $luar)
                            <tr class="luars-row {{ $luarsIndex >= 5 ? 'hidden' : '' }}"
                                data-index="{{ $luarsIndex }}">
                                <td class="border p-2 text-center border-gray-300 dark:border-gray-700 text-sm">
                                    {{ \Carbon\Carbon::parse($luar->created_at)->format('d/m/Y') }}
                                </td>
                                <td class="border p-2 text-center border-gray-300 dark:border-gray-700 text-sm">
                                    {{ $luar->kode ?? '-' }}
                                </td>
                                ...
                                <td class="border p-2 text-right border-gray-300 dark:border-gray-700 text-sm">
                                    {{ number_format($luar->netto ?? 0, 0, ',', '.') }}
                                </td>
                            </tr>
                            @php $luarsIndex++; @endphp
                        @empty
                            <tr>
                                <td colspan="4"
                                    class="border p-2 text-center border-gray-300 dark:border-gray-700 text-sm text-gray-500">
                                    Tidak ada data luar yang sesuai dengan silo "{{ $silo->nama }}"
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div class="mt-3 flex justify-center">
                    <div class="flex items-center gap-2">
                        <label class="text-sm text-gray-600 dark:text-gray-400">Tampilkan:</label>
                        <select id="luars-per-page" onchange="changeLuarsPerPage()"
                            class="px-6 py-1 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
                            <option value="5">5</option>
                            <option value="15">15</option>
                            <option value="25">25</option>
                        ...
                        <span
                            class="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded">
                            <span id="showing-luars">5</span>
                            dari {{ $laporanLuarsTotal }} data
                        </span>
                    </div>
                </div>
            </div>
